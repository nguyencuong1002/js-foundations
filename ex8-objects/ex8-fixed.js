...
var UI = Object.assign(Object.create(Helpers), {
  ...
  init() {
    ...
    this.$workEntrySelectProject = this.$workEntryForm.find(
      "[rel*=js-select-project]"
    );
    this.$workEntryDescription = this.$workEntryForm.find(
      "[rel*=js-work-description]"
    );
    this.$workEntryTime = this.$workEntryForm.find("[rel*=js-work-time]");
    this.$workEntrySubmit = this.$workEntryForm.find(
      "[rel*=js-submit-work-entry]"
    );
  ...
  addWorkEntryToList(project, workEntryData) {
    ...
    $workEntry
      ...
      .text(this.formatTime(workEntryData.time));
    this.setupWorkDescription(
      workEntryData,
  ...
  setupWorkDescription(workEntryData, $workDescription) {
    $workDescription.text(
      this.formatWorkDescription(workEntryData.description)
    );

    if (
      workEntryData.description.length > this.maxVisibleWorkDescriptionLength
    ) {
  ...
  updateProjectTotalTime(project) {
    ...
    $projectEntry
      .find("> [rel*=js-work-time]")
      .text(this.formatTime(projectTime))
      .show();
  },
